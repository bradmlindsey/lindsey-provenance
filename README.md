...
- **A Fail-Closed Commit Gate: Atomic, One-Shot Authorization Bound to Content-Addressed Evidence in a Provenance-Governed Belief Ledger** — specifies and evaluates the fail-closed commit gate that closes the authorization/evidence split-brain: at the durable ledger mutation it atomically re-checks and spends a one-shot, revocable authorization together with the sealed evidence. Against a frozen 626-case suite (464 adversarial) it admits zero unauthorized commits; concurrency and abrupt-crash harnesses confirm single-append idempotence and prior-or-next recovery. A specification and its evaluation, not a deployed-system security claim. https://doi.org/10.5281/zenodo.22012422
